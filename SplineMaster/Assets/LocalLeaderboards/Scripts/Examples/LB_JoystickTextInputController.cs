...
public class LB_JoystickTextInputController : MonoBehaviour
{
	...
	void Start ()
	{
		// grab max length from static length var our leadboard script
		maxLength = LB_Leaderboard.maxLength;
	...
	void GetInput ()
	{
		...
		{
			// look for the delete symbol and react if we need to
			...
			{
				if ( nameText.text.Length < maxLength ) // <-- only advance characters when we haven't yet reached the last character
				{
					AdvanceInput ();
				} else if ( autoSubmitOnMaxLength ) // <-- if autoSubmitOnMaxLength is checked and we're at the last character, we submit the name to the board
				{
					InputCompleted ();
				}
			}
		...
		if ( Input.GetButtonUp ("Fire2") && useFire2ForSubmit )
		{
			InputCompleted ();
		}
	}
	...
	void InputCompleted ()
	{
		// deal with 'empty' (space filled) string
		if ( nameText.text == " " )
			nameText.text = "";
